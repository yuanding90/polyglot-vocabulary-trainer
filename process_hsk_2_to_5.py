#!/usr/bin/env python3
"""
Simple HSK 2-5 Processor

Processes HSK levels 2-5 sequentially with clear progress tracking.
"""
...
class SimpleHSKProcessor:
    ...
    def process_hsk_level(self, hsk_level: int) -> List[Dict]:
        ...
        output_file = f"hsk_api_enhanced_vocabulary/hsk{hsk_level}_api_enhanced.csv"
        ...
        os.makedirs("hsk_api_enhanced_vocabulary", exist_ok=True)
        ...
        for i in range(0, len(all_words), self.words_per_api_call):
            ...
            if results:
                ...
                for result in results:
                    print(f"✅ {result['chinese_word']} → {result['english_translation']}")
    ...
    def process_all(self):
        """Process HSK 2-5 sequentially."""
        print("🎯 Processing HSK 2-5 with Anthropic API")
        print("=" * 60)
        
        all_results = {}
        
        for hsk_level in [2, 3, 4, 5]:
            print(f"\n{'='*20} HSK {hsk_level} {'='*20}")
...
def main():
    ...
    # Process all levels
    processor = SimpleHSKProcessor(api_key)
